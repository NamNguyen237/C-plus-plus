...
ThreeDimPoint::ThreeDimPoint(const ThreeDimPoint &p3) : Point((Point &)p3){
    z = p3.z;
}
ThreeDimPoint & ThreeDimPoint::operator=(const ThreeDimPoint &p3) {
    Point::operator=((Point &)p3);
    z = p3.z;
    return *this;
}
ostream & operator<<(ostream &out, const ThreeDimPoint &p3) {
    out << (Point &)p3; //dua ra kenh xuat out 2 tp x, y
    out << "\nz = " << p3.z;
    return out;
}
istream & operator>>(istream &in, const ThreeDimPoint &p3) {
    in >> (Point &)p3;
    cout << "\nz = ";
    in >> p3.z;
    return in;
